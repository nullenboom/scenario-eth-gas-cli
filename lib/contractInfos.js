		...
		contracts.filter(contract => contract.name != 'Migrations').forEach(contract => {
			...
			contract.methods.forEach(method => {
				...
				method.parameters.forEach(parameter => {
					console.log("Input-Parameter: " + parameter.name + " | " + "Type " + parameter.typeName.name)
				})
				if(method.returnParameters != null){
				method.returnParameters.forEach(returnParameter => {
					console.log("Return-Parameter: Type " + returnParameter.typeName.name)
				})
				}
				console.groupEnd();
			})
